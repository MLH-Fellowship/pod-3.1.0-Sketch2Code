...
When we thought of something like this we were clueless on how to start and build this product. But later with the flow we tried various techniques, various languages and got one which would suit our requirements. Eventually with time we made the normal procedure more easy and user friendly to developers by adding Machine Learning technology into S2C. 

Later when we realised that it was not easy for all to draw Sketch of UI and provide it we thought to bring Text 2 Code. So Text 2 Code is an even easier way where user can just type-in their requirements and the code wil be generated.
